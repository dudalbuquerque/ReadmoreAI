class USER:
    ...
    def register_user(self, user_name, user_age, user_email, user_password):
            ...
            self.db.cursor.execute(
                """
                INSERT INTO Readmore_users(name, age, email, password, registration_date)
                VALUES (?, ?, ?, ?, CURRENT_DATE);
                """, (user_name, user_age, user_email, user_password)
            )
            self.db.conn.commit()
    ...
    def esqueceu_senha(self, user_id, new_password):
        query = "UPDATE Readmore_users SET senha = ? WHERE id = ? LIMIT 1;"
